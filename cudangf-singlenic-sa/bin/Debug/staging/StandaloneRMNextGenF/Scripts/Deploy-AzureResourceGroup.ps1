...
Param(
  ...
  [string] $ResourceGroupName = 'BarracudaWAF',
...
If(!(Get-Command -Name "Switch-AzureMode" -ErrorAction SilentlyContinue)){
    Write-Host "Running in Powershell v1 mode"
...
New-AzureRmResourceGroupDeployment -Name "Deploy_$($ResourceGroupName)" -ResourceGroupName $ResourceGroupName `
    ...
    Write-Host "Still running the old stuff"
